...
| 规则 | 条件 | 效果 |
|---|---|---|
| 同内容降噪 | CONTENT_SAME | 撤销一次modify计数 |
...
事件表不能无限增长——PostgreSQL 是平台共享实例，GFRGuard 不能成为平台的存储负担。保留策略：**数据库只保留最近 10000 条、且不超过 30 天的事件**；每日归档任务将超出保留窗口的事件导出为 CSV（需要压缩成zip），随后从库中删除。
