...
| 特性 | 说明 |
| --- | --- |
| 📋 许可激活 | 通过"自动获取license"操作导入许可，启用防勒索功能 |
| 📋 许可激活 | 通过"手动输入license"操作导入许可，启用防勒索功能 |
| 📋 许可状态 | 展示激活状态（未激活/已激活）与有效期 |
| 📋 许可状态 | 没有有效的license时，勒索软件检查开关为禁用状态，无法点击 |
...
| 📋 释放隔离区磁盘空间 | 隔离区所占磁盘空间将被释放，勒索功能关闭才能点击 |
...
| 📋 释放安全备份区磁盘空间 | 安全备份区所占磁盘空间将被释放，勒索功能关闭才能点击 |
...
| 📋 勒索事件显示上限与 CSV 归档 | events 表仅保留最近 10000 条 / 30 天，超出每日归档为 CSV 后删库：当日无事件不生成 CSV（1 条系统日志）；≤10000 生成 1 个 CSV；>10000 按每文件最多 10000 条拆分多个 CSV；每个 CSV 输出已处理/未处理两条系统日志，csv压缩成zip格式 |
