...
**FR-DAEMON-03: 内容信号分析**
- **描述**：对内容进行 Shannon 熵采样和内容规则匹配，结果作为行为风险评分的加权输入维度，与文件操作行为指标（modified/rename/delete 等）同级。不逐文件标记 damaged/corrupted/encrypted 状态，不累计"损坏/加密文件数"，不按"某类文件数量≥N"的方式阻断。
- **核心规则**：
  - 触发时机：写入过程中（SMB=pwrite、fanotify=FAN_MODIFY），不在文件关闭后执行
  - 采样去重：同一文件仅采样一次（首次收到写事件时），后续重复写入不重复检测
  - 会话去重：同一会话首次命中内容信号后，该会话后续文件不再重复检测
...
**FR-CONSTRAINT-03: 内容信号定位**

内容检测（熵分析、规则匹配）在写入过程中触发，结果作为行为风险评分的加权输入维度。系统不做文件访问结束后的内容质量/损坏判定，不逐文件标记 damaged/corrupted 状态，不累计"损坏文件数量"，也不单独按"某类文件数≥N"触发阻断。
